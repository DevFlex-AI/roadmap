...
Features are mostly complete and documented. The timeline and requirements for GA are usually published. No SLAs or support provided.
...
- **in design:**\
Feature in the discovery phase. We have decided to build this feature, but are still figuring out _how_.
...
Some features may be marked with a GHES  3. X label, which indicates that the feature will also become available to GitHub Enterprise Server customers. Below are the release version numbers and expected release quarters (_Note: these dates are subject to change_). 
...
GitHub Enterprise Server has major releases on a quarterly basis and minor releases on a monthly basis. Once we know which version we are delivering a feature, we will update the issue to indicate that information.
